...
class Agent {
  ...
  **/
  public function new(color: Int) {
    ...
    position = lastPosition = lastBounce = [
  ...
  **/
  ...
  private function emitLine(graphics: h2d.Graphics) {
    ...
    if (len < MIN_LENGTH) { return; } // don't render short lines
  ...
  **/
  private function clamp(a: Float, start: Float, end: Float) {
    if (a <= start) { return { at: start, inRange: false }; }
    else if (a >= end) { return {at: end, inRange: false }; }
    else { return {at: a, inRange: true}; }
  }
  ...
  **/
  private function crossedAnyLines(pixels: hxd.Pixels) : Bool {
    ...
    if (pixelAt(position, pixels) != 0x000000) { return true; }
