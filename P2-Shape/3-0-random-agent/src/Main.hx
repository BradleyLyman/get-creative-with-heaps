...
class Main extends hxd.App {
  ...
  override function update(dt:Float) {
    ...
    graphics[current].beginFill(
      colorFor(lerp(current / graphics.length, Math.PI / 2, 3 * Math.PI / 2)),
      0.2
    );
    for (agent in agents) {
  ...
  function resetAgents() {
    ...
    agents = [
      for (x_ind in 0...cols) {
        for (y_ind in 0...rows) {
          new Agent(
            ((s2d.width - w) / 2) + (x_ind * gridSize),
            ((s2d.height - h) / 2) + (y_ind * gridSize)
          );
        }
      }
    ];
