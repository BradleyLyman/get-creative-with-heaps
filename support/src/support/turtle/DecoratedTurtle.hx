...
class DecoratedTurtle implements Turtle {
  ...
  private function set_position(v) {
    return wrapped.position = v;
  }

  private function get_lineWidth() {
    return wrapped.lineWidth;
  }

  private function set_lineWidth(w) {
    return wrapped.lineWidth = w;
  }
  ...
  private function set_color(c) {
    return wrapped.color = c;
  }
}
