...
class Plot extends Object {
  ...
  **/
  public function drawQuad(quad:Quad, ?color:Color) {
    final transformed:Quad = new Quad(
      space.map(quad.topLeft),
      space.map(quad.topRight),
      space.map(quad.bottomLeft),
      space.map(quad.bottomRight)
    );
    if (color != null) {
  ...
  **/
  ...
  private function set_xAxis(axis) {
  ...
  private function set_yAxis(axis) {
